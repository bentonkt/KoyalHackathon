...
function ImportDialog({ project, onClose, onAccept }: { project: Project; onClose: () => void; onAccept: (p: Project) => void }) {
  ...
  function accept() { try {
    ...
    const track = importCandidate(candidate, { role, stageId: project.stage.id, sourceWidth: width, sourceDepth: depth, stageWidth: project.stage.width, stageDepth: project.stage.depth });
  ...
  return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-labelledby="import-title"><button className="modal-close" aria-label="Close import" onClick={onClose}><X size={18}/></button><span className="section-label">CV → DIRECTOR'S DESK</span><h2 id="import-title">Bring physical motion on stage.</h2><p>Import Benton's <code>candidates/*.json</code> output after tracking with <code>--corners</code>. This is a review step, not object selection or calibration.</p>
    <label className="file-picker"><ArrowUpFromLine size={22}/><strong>{filename || 'Choose a trajectory candidate'}</strong><span>Local JSON only · no upload to a server</span><input type="file" accept=".json,application/json" onChange={async e => { const f = e.target.files?.[0]; if (!f) return; setCandidate(null); setReviewed(false); setError(''); try { if (f.size > 10_000_000) throw new Error('Candidate exceeds 10 MB.'); setCandidate(JSON.parse(await f.text())); setFilename(f.name); } catch (err) { setError((err as Error).message); } }}/></label>
    ...
    <div className="two-fields"><label className="field-label">CV stage width<input type="number" min=".01" step=".1" value={width} onChange={e => setWidth(+e.target.value)}/></label><label className="field-label">CV stage depth<input type="number" min=".01" step=".1" value={depth} onChange={e => setDepth(+e.target.value)}/></label></div>
    <p className="microcopy">The current Python CLI exports 1 × 1. Change these only if Benton changes the geometry function's stage dimensions. Mapped to the {project.stage.width} × {project.stage.depth} virtual stage; this is reviewed artistic scale, not meters. Heading is fixed/look-at.</p>
    <label className="checkbox"><input type="checkbox" checked={reviewed} onChange={e => setReviewed(e.target.checked)}/> I reviewed these extents and confirm this candidate belongs to this project's unchanged physical stage ({project.stage.id}).</label>
    ...
    <p className="microcopy">Pair Actor A and B from the same Python take ID. Camera may use a separate take; align its start with the offset control after import. Source evidence remains in the Python project.</p>
